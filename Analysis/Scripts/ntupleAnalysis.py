...
studies = ['MagField', 'OverlapsHalfModules', 'Overlaps', 'ClusterSize', 'FullLayers', 'Outliers', 'PositionsHalfModules', 'PositionsSectors']
studies = ['PositionsSectors']
...
if __name__ == '__main__':
    ...
    if 'Outliers' in studies:
        ...
        for (i,hm) in enumerate(outliers['DeltaY']):
            ...
            outliers['DeltaY'][hm]['ClusterZ'] = r.TH1F('CZForYOutliers_%s'%hm,'CZForYOutliers_%s'%hm,10,min(cz),max(cz))
            [ outliers['DeltaY'][hm]['ClusterZ'].Fill(z) for z in cz ]
            outliers['DeltaY'][hm]['ClusterX'] = r.TH1F('CXForYOutliers_%s'%hm,'CXForYOutliers_%s'%hm,10,min(cx),max(cx))
            [ outliers['DeltaY'][hm]['ClusterX'].Fill(x) for x in cx ]
            outliers['DeltaY'][hm]['ClusterY'] = r.TH1F('CYForYOutliers_%s'%hm,'CYForYOutliers_%s'%hm,10,min(cy),max(cy))
            [ outliers['DeltaY'][hm]['ClusterY'].Fill(y) for y in cy ]
            outliers['DeltaY'][hm]['TrackZ'] = r.TH1F('TSZForYOutliers_%s'%hm,'TSZForYOutliers_%s'%hm,10,min(tsz),max(tsz))
            [ outliers['DeltaY'][hm]['TrackZ'].Fill(z) for z in tsz ]
            outliers['DeltaY'][hm]['TrackX'] = r.TH1F('TSXForYOutliers_%s'%hm,'TSXForYOutliers_%s'%hm,10,min(tsx),max(tsx))
            [ outliers['DeltaY'][hm]['TrackX'].Fill(x) for x in tsx ]
            outliers['DeltaY'][hm]['TrackY'] = r.TH1F('TSYForYOutliers_%s'%hm,'TSYForYOutliers_%s'%hm,10,min(tsy),max(tsy))
            ...
            for hm in listOfHM:
                for gr in histosByHM[hm].keys():
                    if isinstance(histosByHM[hm][gr], r.TGraph):
                        ...
                        label.SetTextFont(132); label.SetTextSize(0.06)
                        label.AddText(hm); label.Draw('same'); c1.Modified(); c1.Update()
                        c1.SaveAs('../Out/'+shortFilename+'/%s/%s/PlotsByHalfModule/%s'%(tracker, datatype, hm) + '-%s.png'%gr)
                        #c1.Write()
                        c1.Close()
                    elif isinstance(histosByHM[hm][gr], list):
                        if len(histosByHM[hm][gr])>0: # skip empty modules
                            h = r.TH1F('%s_%s'%(gr, hm), '%s_%s'%(gr, hm), 100, min(histosByHM[hm][gr]), max(histosByHM[hm][gr]))
            ...
            for sector in flatDetector:
                for gr in histosBySector[sector].keys():
                    if isinstance(histosBySector[sector][gr], r.TGraph):
                        ...
                        label.SetTextFont(132); label.SetTextSize(0.06)
                        label.AddText(sector); label.Draw('same'); c1.Modified(); c1.Update()
                        c1.SaveAs('../Out/'+shortFilename+'/%s/%s/PlotsBySector/%s'%(tracker, datatype, sector) + '-%s.png'%gr)
                        #c1.Write()
                        c1.Close()
                    elif isinstance(histosBySector[sector][gr], list):
                        if len(histosBySector[sector][gr])>0: # skip empty modules
                            h = r.TH1F('%s_%s'%(gr, sector), '%s_%s'%(gr, sector), 100, min(histosBySector[sector][gr]), max(histosBySector[sector][gr]))
