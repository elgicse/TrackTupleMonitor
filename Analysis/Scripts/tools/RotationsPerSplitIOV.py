...
def RotationsPerSplitIOV(tracker, datatype, listOfHM, t, save, shortFilename, segment, detector, lookup):
    if tracker == 'IT':
        print 'Sorry I cannot do this for the IT, yet...'
        sys.exit(0)
    if not os.path.exists('../Out/'+shortFilename+'/%s/%s/Histograms_SplitIOVs'%(tracker, datatype)):
        os.system('mkdir ../Out/'+shortFilename+'/%s/%s/Histograms_SplitIOVs'%(tracker, datatype))
    if not os.path.exists('../Out/'+shortFilename+'/%s/%s/PlotsByHalfModule_SplitIOVs'%(tracker, datatype)):
        os.system('mkdir ../Out/'+shortFilename+'/%s/%s/PlotsByHalfModule_SplitIOVs'%(tracker, datatype))
    #
    # Book Histograms
    histosByHM = {}
    rotations = {}
    ...
    for hm in listOfHM:
        histosByHM[hm] = { 
            ...
            'AvgResidualByIOV': r.TH1F('AvgResidualByIOV_'+hm, 'AvgResidualByIOV_'+hm, len(SplitIOVs_instance.intervals), 0, 1),
            'ResidualRMSByIOV': r.TH1F('ResidualRMSByIOV_'+hm, 'ResidualRMSByIOV_'+hm, len(SplitIOVs_instance.intervals), 0, 1),
            'RxByIOV': r.TH1F('RxByIOV_'+hm, 'RxByIOV_'+hm, len(SplitIOVs_instance.intervals), 0, 1)
        }
        for i in SplitIOVs_instance.intervals:
    ...
    for hm in listOfHM:
        #avg, rms = [], []
        rotations[hm] = {}
        for iov in xrange(len(SplitIOVs_instance.intervals)):
            rotations[hm][iov] = {}
            ...
            histosByHM[hm]['ResidualRMSByIOV'].GetXaxis().SetBinLabel(iov+1, SplitIOVs_instance.intervals[iov]['start'].replace('2012-','') + ' to ' 
                + SplitIOVs_instance.intervals[iov]['end'].replace('2012-',''))
            histosByHM[hm]['AvgResidualByIOV'].GetXaxis().SetBinLabel(iov+1, SplitIOVs_instance.intervals[iov]['start'].replace('2012-','') + ' to ' 
                + SplitIOVs_instance.intervals[iov]['end'].replace('2012-',''))
            if histosByHM[hm]['HitZvsTrackYByIOV-'+str(iov)].GetN() > 2:
                ...
                if save:
                    c1.SaveAs('../Out/'+shortFilename+'/%s/%s/PlotsByHalfModule_SplitIOVs/%s-HitZvsTrackY-IOV%s.png'%(tracker, datatype, hm, iov))
                c1.Close() #debug
            ...
            histosByHM[hm]['RxByIOV'].GetXaxis().SetBinLabel(iov+1, SplitIOVs_instance.intervals[iov]['start'].replace('2012-','') + ' to ' 
                + SplitIOVs_instance.intervals[iov]['end'].replace('2012-',''))
    # Style up
    ...
    if save:
        ...
        outfile = r.TFile('../Out/'+shortFilename+'/%s/%s/Histograms_SplitIOVs/RotationsByIOV-%s.root'%(tracker, datatype, segment), 'recreate')
        for hm in listOfHM:
