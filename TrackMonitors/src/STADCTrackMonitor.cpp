...
StateVector* STADCTrackMonitor::GetTTHit(const LHCb::Track* track)
{
  ...
  const DeSTDetector::Stations& stations = m_STDet->stations();
  double zTTa   =  stations[0]->globalCentre().z();
  double zTTb   =  stations[1]->globalCentre().z();

  const State& TTaState = track->closestState(zTTa);
  const State& TTbState = track->closestState(zTTb);
  StateVector* stateVectorTT;

  if(std::string::npos != m_layer.find("TTa")){
    stateVectorTT = new StateVector(TTaState.position(), TTaState.slopes());
  }
  else{
    stateVectorTT = new StateVector(TTbState.position(), TTbState.slopes());
  }
...
{
  const DeSTDetector::Stations& stations = m_STDet->stations();
  double zIT1   =  stations[0]->globalCentre().z();
  double zIT2   =  stations[1]->globalCentre().z();
  double zIT3   =  stations[2]->globalCentre().z();

  const State& IT1State = track->closestState(zIT1);
  const State& IT2State = track->closestState(zIT2);
  const State& IT3State = track->closestState(zIT3);
  StateVector* stateVectorIT;

  if(std::string::npos != m_layer.find("IT1")){
    stateVectorIT = new StateVector(IT1State.position(), IT1State.slopes());
  }
  else if(std::string::npos != m_layer.find("IT2")){
    stateVectorIT = new StateVector(IT2State.position(), IT2State.slopes());
  }
  else{
    stateVectorIT = new StateVector(IT3State.position(), IT3State.slopes());
  }
